...
void Camera::aim(const vec3 &look_at) { aim(look_at, vertical); }
...
void Camera::focus(float focal_dist, float aspect_ratio, float fov) {
  this->focal_dist = focal_dist;
  float width = 2.0 * focal_dist * tan(0.5 * fov);
...
unsigned int Camera::pixel_color(const vec3 &color) {
  unsigned int r = std::min(color[0], 1.0f) * 255;
  unsigned int g = std::min(color[1], 1.0f) * 255;
  unsigned int b = std::min(color[2], 1.0f) * 255;
